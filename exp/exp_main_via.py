    ...
    def train(self, setting):
        ...
        for epoch in range(self.args.train_epochs):
            ...
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(train_loader):
                ...
                x_g1_r, x_g2_r, x_g3_r = reshape_input(x_g1), reshape_input(x_g2), reshape_input(x_g3)
                y_g1_r, y_g2_r, y_g3_r = reshape_input(y_g1), reshape_input(y_g2), reshape_input(y_g3)
                ...
                if self.args.use_amp:
                    with torch.cuda.amp.autocast():
                        if 'Linear' in self.args.model:
                            # outputs = self.model(batch_x)
                            out_g1 = self.model_group1(x_g1)
                            out_g2 = self.model_group2(x_g2)
                            out_g3 = self.model_group3(x_g3)
                        else:
                            if self.args.output_attention:
                                # outputs = self.model(batch_x, batch_x_mark, dec_inp, batch_y_mark)[0]
